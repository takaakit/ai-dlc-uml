...
**CRITICAL**: When performing any phase, you MUST read and use relevant content from rule detail files under this path:
- `.aidlc-rule-details/`
...
- Load `common/uml-modeling-guide.md` for the prerequisites and modeling guidelines.
...
## MANDATORY: UML Modeling Guide
**CRITICAL**: Before creating, updating, or reviewing UML model elements and diagrams in Astah Pro, you MUST read and follow `common/uml-modeling-guide.md`.

## MANDATORY: Language Selection
**CRITICAL**: If the user does not explicitly specify a language to use (such as English or Spanish), use the same language as the user's prompt for chat messages, questions, plans, and all other artifacts and documentation.
...
4. **MANDATORY**: Load UML modeling guide from `common/uml-modeling-guide.md`
...
- Per-Unit Loop (executes for each unit):
  ...
  - Design-Code Synchronization (CONDITIONAL, per-unit)
- Build and Test (ALWAYS - after all units complete)

**Note**: Each unit is completed fully (design + code + design-code synchronization) before moving to the next unit.
...
**Execute IF**:
- Design artifacts exist for this unit

**Skip IF**:
- No design artifacts exist for this unit

**Purpose**: Verify that the generated/modified code for the unit is fully consistent with all construction-phase artifacts produced for that unit (design models and documents). Eliminate any drift before moving on — this stage enforces the Synchronization principle (see Key Principles).

**Execution**:
1. **MANDATORY**: Log any user input during this stage in audit.md
2. Load all steps from `construction/design-code-synchronization.md`
3. Perform diff analysis between the unit's code and the unit's construction-phase design artifacts
4. **If no differences are detected**:
   - Record the result in the sync report
   - Present standardized 2-option completion message
5. **If differences are detected**:
   - Create a question file listing ALL differences (never omit any) and asking the user, per difference, to choose whether to update the design artifact or update the code
   - Wait for the user to answer every item (do not proceed with unresolved items)
   - Apply the user's choices:
     - **Design-update items** → update the corresponding UML model elements and diagrams, and/or design documents to match the code
     - **Code-update items** → re-enter `Code Generation (ALWAYS EXECUTE, per-unit)` Part 2 for this unit to regenerate/modify the affected code based on the user's choices, then return to Design-Code Synchronization (this stage re-runs until no differences remain)
   - Loop until the diff set is empty
6. **MANDATORY**: Present standardized 2-option completion message as defined in design-code-synchronization.md - DO NOT use emergent behavior
...
- **UML Modeling**: Application and functional design outputs shall be expressed as UML models, which will be iteratively refined with users through UML modeling.
- **Synchronization**: Code shall be generated or updated based on the UML models and design documents. In all cases where the code is modified, whether by an AI agent or a user, the corresponding UML models and associated design documents shall be updated without exception to ensure full synchronization with the code.
...
│   │   ├── requirements/
│   │   ├── user-stories/
│   │   └── application-design.asta # Astah Pro project file
│   ├── construction/               # 🟢 CONSTRUCTION PHASE
│   │   ├── plans/
│   │   ├── functional-design.asta  # Astah Pro project file
│   │   ├── {unit-name}/
│   │   │   ├── nfr-requirements/
│   │   │   ├── nfr-design/
│   │   │   ├── infrastructure-design/
│   │   │   ├── code/               # Markdown summaries only
│   │   │   └── design-code-sync/   # Design-code synchronization artifacts
│   │   └── build-and-test/
