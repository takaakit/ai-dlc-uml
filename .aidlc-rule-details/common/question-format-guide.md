...
### Rule: Open Question Files Automatically
**CRITICAL**: After creating a question file, you MUST execute a shell command to open the file for the user.
...
#### Step 2: Open Question File

**MANDATORY**: Immediately after creating the question file, and BEFORE informing the user (Step 3), open the file in the OS default application so the user does not have to locate and open it manually.

**How to open**: Detect the operating system from the environment, then run the matching command. Substitute the path placeholder with the full absolute path of the file to open.

| Environment         | Command                                                                                    |
| ------------------- | ------------------------------------------------------------------------------------------ |
| Windows PowerShell  | `Start-Process "<windows-full-path-to-file>"`                                              |
| Windows cmd.exe     | `start "" "<windows-full-path-to-file>"`                                                   |
| Windows WSL2 Ubuntu | `/mnt/c/Windows/System32/cmd.exe /c start "" "$(wslpath -w "<ubuntu-full-path-to-file>")"` |
| macOS               | `open "<macos-full-path-to-file>"`                                                         |
| Linux               | `xdg-open "<linux-full-path-to-file>"`                                                     |

**Notes**:
- Run the open command exactly once per question file creation.
- If the open command fails (non-GUI environment, no registered handler, etc.), continue to Step 3 — do NOT block the workflow on an open failure.

#### Step 3: Inform User
```
"I've created and opened {phase-name}-questions.md with [X] questions. 
Please answer each question in the opened document by filling in the letter choice after the [Answer]: tag. 
