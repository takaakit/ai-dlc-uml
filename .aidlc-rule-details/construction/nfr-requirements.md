...
- Open the Astah Pro project `aidlc-docs/construction/functional-design.asta`, and fully understand the functional design represented by the model elements (including the description field text) and diagrams (including the notes in the diagrams) under the following packages: `{unit-name}/business-logic`, `{unit-name}/domain-model`, and `{unit-name}/frontend-components`.
